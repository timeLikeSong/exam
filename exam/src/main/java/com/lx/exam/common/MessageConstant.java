package com.lx.exam.common;

public class MessageConstant {
	public static class STATUS{
		public final static String ADD_SUCCESS="ADD_SUCCESS";
		public final static String ADD_FAILURE="ADD_FAILURE";
		
		public final static String DEL_SUCCESS="DEL_SUCCESS";
		public final static String DEL_FAILURE="DEL_FAILURE";
		
		public final static String EDIT_SUCCESS="EDIT_SUCCESS";
		public final static String EDIT_FAILURE="EDIT_FAILURE";
		
		
		public final static String FOUND="FOUND";
		public final static String NOT_FOUND="NOT_FOUND";
		public final static String EXCEPTION="EXCEPTION";
		public final static String PARAM_ERROR="PARAM_ERROR";
	}
	public static class MESSAGE{
		public final static String ADD_SUCCESS="添加成功";
		public final static String ADD_FAILURE="添加失败";
		
		public final static String DEL_SUCCESS="删除成功";
		public final static String DEL_FAILURE="删除失败";
		
		public final static String EDIT_SUCCESS="修改成功";
		public final static String EDIT_FAILURE="修改失败";
		
		public final static String NOT_FOUND="没有记录";
		public final static String EXCEPTION="服务器忙";
		public final static String PARAM_ERROR="参数错误";
	}
}
