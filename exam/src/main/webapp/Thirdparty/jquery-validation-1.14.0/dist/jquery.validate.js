 ...
 * 
 * Copyright (c) 2015 Jörn Zaefferer Released under the MIT license
 */
(function(factory) {
		...
		prototype : {
			...
			init : function() {
				...
				function delegate(event) {
					var validator = $.data(this.form, "validator"), eventType = "on"
				...
				$(this.currentForm)
						...
						// "select" is provided as event.target when clicking a
						// option
						.on(
								"click.validate",
				...
				if (this.settings.invalidHandler) {
					$(this.currentForm).on("invalid-form.validate",
			...
			resetForm : function() {
				...
				this.prepareForm();
				this.hideErrors();
				var i, elements = this.elements().removeData("previousValue")
			...
			showLabel : function(element, message) {
				var place, group, errorID, error = this.errorsFor(element), elementID = this
						.idOrName(element), describedBy = $(element)
				...
				if (error.length) {
					...
					error = $("<" + this.settings.errorElement + ">").attr(
							"id", elementID + "-error")
							.addClass(this.settings.errorClass).html(message
									|| "");

					// Maintain reference to the element to be placed into the
					// DOM
					place = error;
					if (this.settings.wrapper) {
					...
					if (error.is("label")) {
						...
						// If the element is not a child of an associated label,
						// then it's necessary
						...
						if (!describedBy) {
							describedBy = errorID;
						} else if (!describedBy.match(new RegExp("\\b"
								+ errorID + "\\b"))) {
						...
						// If this element is grouped, then assign to all
			...
			errorsFor : function(element) {
				var name = this.idOrName(element), describer = $(element)
						.attr("aria-describedby"), selector = "label[for='"
						+ name + "'], label[for='" + name + "'] *";
			...
			// cleans up all forms and elements, removes validator-specific
			// events
			destroy : function() {
				this.resetForm();
